...
| `RawDarksFolder` | string | _(empty)_ | Optional: raw DARK FITS root for stacker, same-night counts, Auto save, and raw purge; when empty, NINA image path is used (narrowed to DARK subtree when the file pattern places `$$IMAGETYPE$$` in a path folder) |
